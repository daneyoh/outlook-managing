@echo off
REM Install autostart WITHOUT admin / Task Scheduler.
REM Puts a small launcher in the Windows Startup folder so the
REM background fetcher (fetch_loop.py) starts at every logon.

set "STARTUP=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
set "VBS=%STARTUP%\OutlookMailFetch.vbs"
...
>>"%VBS%" echo sh.Run """%PYW%"" ""%~dp0..\00. BACKEND\fetch_loop.py""", 0, False

if exist "%VBS%" (
  echo [DONE] Autostart installed.
  echo  - It starts automatically at the next logon/boot.
  echo  - To start NOW without rebooting: double-click 실행\start_now.bat
  echo  - To remove: double-click 설치\uninstall_autostart.bat
) else (
